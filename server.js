...
// This holds the single shared dataset that ALL portal users see.
// Data persists as long as the server process is running.
// On first connect after restart, clients re-upload from their localStorage.

const store = {
  leads: [],
  prospects: [],
  customers: [],
  version: 0,            // Increments on every write
  lastUpdate: Date.now()
};

const events = [];  // { id, type, data, ts }
let eventIdCounter = 1;

// ─── Merge Helper ────────────────────────────────────────────────────────────
// Merges two arrays by `id` field. Items from `incoming` overwrite `existing`
// when they share an ID; new IDs are added. This gives "last-write-wins" per item.
...
// ─── POST /api/data — Client pushes its local data, receives merged result ───
app.post('/api/data', (req, res) => {
  try {
    const { leads, prospects, customers } = req.body;

    if (leads && Array.isArray(leads)) {
      store.leads = mergeById(store.leads, leads);
    }
    if (prospects && Array.isArray(prospects)) {
      store.prospects = mergeById(store.prospects, prospects);
    }
    if (customers && Array.isArray(customers)) {
      store.customers = mergeById(store.customers, customers);
    }
...
// ─── GET /api/data — Client polls for current shared data ────────────────────
app.get('/api/data', (req, res) => {
  ...
  if (sinceVersion >= store.version) {
    return res.json({ ok: true, changed: false, version: store.version });
  }
...
// ─── POST /api/event — Landing pages push events (proposal accepted, etc.) ──
app.post('/api/event', (req, res) => {
  try {
    ...
    const evt = {
      id: eventIdCounter++,
      type: type,  // 'proposal_accept', 'agreement_sign', 'callback_request'
      data: data,
      ts: Date.now()
    };
    events.push(evt);

    // Keep events for 24 hours max, and limit to 500 entries
...
// ─── GET /api/events — Portal polls for new events since last check ──────────
app.get('/api/events', (req, res) => {
  ...
  res.json({
    ok: true,
    events: newEvents,
    lastId: events.length > 0 ? events[events.length - 1].id : 0
  });
});
